...
int get_rows(xlsxioreadersheet sheet, char **query, int *index, char *table) {
    ...
    if (!sheet || !query || !index || !bq)
        return 1;
    bzero(buff, 100);
    snprintf(buff, 100, I_QUE, bq);
...
int import_xlsx(char *filename, char *table) {
    ...
    if (!filename || !table)
        return 1;
    workbook = xlsxioread_open(filename);
    if (!workbook)
    ...
    get_rows(sheet, &query, index, table);
