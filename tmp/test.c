...
int main(int argc, char *argv[]) {
    gtk_init(&argc, &argv);
    const char* test;
    test = g_get_user_special_dir(G_USER_DIRECTORY_DOCUMENTS);
    printf("%s\n", test);
    // Get the user's documents directory path
    gchar *documentsPath = g_build_filename(g_get_user_special_dir(G_USER_DIRECTORY_DOCUMENTS), NULL);

    g_print("Documents Path: %s\n", documentsPath);

    g_free(documentsPath);
